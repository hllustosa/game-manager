...
const GAMELOANS_GAMES_TL_ENDPOINT = "/api/GameLoans/games/{0}";
...
export function GetLoansTimeLine(friendId, gameId) {
  ...
  if (friendId) {
    const addr = GAMELOANS_FRIENDS_TL_ENDPOINT.replace("{0}", friendId);
    return Axios.get(addr, { headers });
  }

  if (gameId) {
    const addr = GAMELOANS_GAMES_TL_ENDPOINT.replace("{0}", gameId);
    return Axios.get(addr, { headers });
  }
}
